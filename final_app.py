...
def do_reset():
    # Reset everything to defaults and force a fresh run so widgets rebuild cleanly
    reset_state()
    st.rerun()
...
with st.form("calc_form"):
    ...
    with colB:
        # Reset now uses a callback that resets and immediately reruns the app
        st.form_submit_button("Reset", use_container_width=True, on_click=do_reset)

# Note: no post-form "if reset:" block is needed anymore because do_reset() reruns immediately.
